...
把 qcloop 当作执行编排器使用：外层 AI agent 负责理解当前任务、拆分测试项、创建批次、启动运行、监控证据、必要时向人类确认并写回答案，最后汇报结果。qcloop 负责队列、重试/返修轮次、待确认状态、终态判断和 Web 可视化。
...
4. 使用 `POST /api/jobs` 创建批次，再用 `POST /api/jobs/run` 启动。
...
7. 汇报批次 ID、Web 地址、统计数字、失败/已耗尽/待确认证据，以及是否需要后续修复。
...
- `prompt_template` 必须包含 `{{item}}`；verifier prompt 可使用 `{{stdout}}`、`{{stderr}}`、`{{exit_code}}`、`{{attempt_status}}`、`{{attempt_type}}`。
