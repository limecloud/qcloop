...
function jobHelp() {
  ...
  qcloop-skill job create --file <payload.json> [--run] [--mode auto]
...
function jobCounts(items) {
  const counts = { total: items.length, success: 0, failed: 0, exhausted: 0, awaiting_confirmation: 0, running: 0, pending: 0 };
...
function problemItems(items, limit = 20) {
  const problems = [];
  for (const item of items) {
    if (!["failed", "exhausted", "awaiting_confirmation"].includes(item.status)) continue;
...
async function cmdJob(baseUrl, args) {
  ...
  if (sub === "create") {
    ...
    emit(envelope("job create", job, { run_result: runResult, dashboard_url: `${baseUrl}/?job_id=${job.id}` }));
    return 0;
  }
  if (sub === "run") {
