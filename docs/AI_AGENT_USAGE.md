...
如果当前 AI agent 支持仓库内技能，可以启用 [`skills/qcloop`](../skills/qcloop/SKILL.md)。这个技能内置 `qcloop-skill` CLI，参考 Lime CLI 的结构化命令风格，给 agent 使用而不是给普通用户手敲。CLI 也准备为 npm 包 `@limecloud/qcloop-skill-cli`，安装后可直接运行同名命令：
...
AI agent：通过 Skill 或 llm-full.txt 理解当前任务 + 设计 items + 创建/启动批次 + 轮询 + 必要时提问并写回答案 + 总结结果
