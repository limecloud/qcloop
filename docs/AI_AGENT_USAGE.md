...
这篇文档面向 qcloop 的真实使用方式：**人先打开 qcloop 应用，然后让 Codex / Claude Code / Gemini CLI / Kiro CLI 等 AI agent 通过 Skill 或 `llm.txt` 自动创建、运行、观察批量测试任务**。

推荐路径不是让人手动敲命令、手动填 UI 或手动拼 API JSON。人只负责打开应用、授权 agent、给出意图和必要确认；提单、启动、轮询、确认写回、重试和总结由 AI agent 完成。
...
### 技能版：支持 Skill 的 agent 优先用这条

如果当前 AI agent 支持仓库内技能，可以启用 [`skills/qcloop`](../skills/qcloop/SKILL.md)。这个技能内置 `qcloop-skill` CLI，参考 Lime CLI 的结构化命令风格，给 agent 使用而不是给普通用户手敲。CLI 也已整理为 npm 包 `@limecloud/qcloop-skill-cli`，安装后可直接运行同名命令：
...
技能 CLI 默认输出 JSON；失败时 stderr 也会输出 `error_code`、`retryable`、`hint`，方便 AI 决定是等待、重试、还是提示人类先打开 qcloop。

文件型批量任务不需要人类逐行整理 items。外层 AI 可以直接让 CLI 从工作区导入：

- `--items-dir docs`：递归导入目录文件，自动跳过 `.git`、`node_modules`、`dist` 等噪音目录。
- `--glob "docs/**/*.md"`：按 glob 导入文档、代码或测试文件。
- `--git-diff HEAD`：按 `git diff --name-only HEAD` 导入当前变更文件。

这些导入项会保存为结构化 JSON item，包含 `target`、`cwd`、`source` 和 `expected`，适合“146 个文档逐个 review/修复”这类下班托管任务。

如果只有单个 item 需要处理，AI 应优先使用 `item retry` 或 `item cancel`；如果需要判断队列是否真的在跑，使用 `queue metrics`；如果是常见任务配置，使用 `template` 命令复用模板，而不是让人反复手动填 UI。

### llm.txt 版：没有 Skill 时用这条

如果当前 agent 不支持 Skill，就让它读取：

```text
http://localhost:3000/llm-full.txt
```

它会按同一套契约直接调用 HTTP API。两条路径能力等价：Skill 只是把 API 包成更稳定的 JSON CLI。
...
- 如果只是本机 CLI 启动、进程异常、临时不可用，创建批次时可设置 `max_executor_retries`，推荐 `1`；它只用于基础设施错误，不会把目标测试失败当成可重试。
...
- 批次是否进入 `running` / `completed` / `failed` / `canceled`；`completed` 表示全部 item 成功，`failed` 表示批次已结束但存在失败或耗尽项，`canceled` 表示已明确终止且不会恢复
...
- 如果出现 `awaiting_confirmation`，推荐让外层 AI 读取问题并向你确认，然后它会调用 `qcloop-skill item answer ... --resume` 或 `/api/items/answer` 继续；你不需要逐项操作。
- 如果睡前托管跑完，推荐让外层 AI 调 `qcloop-skill job report <job_id> --format markdown` 汇总耗时、统计、问题项和待确认项。
- 如果方向错了或任务不再需要，让外层 AI 调 `qcloop-skill job cancel <job_id> --reason "..."`；取消是不可恢复终态，区别于暂停。
...
```text
人：打开 qcloop + 授权 Codex / Claude Code / Gemini CLI / Kiro CLI + 发一句最小提示词 + 必要时确认风险/歧义
AI agent：通过 Skill 或 llm-full.txt 理解当前任务 + 从上下文/目录/glob/git diff 设计 items + 创建/启动批次 + 轮询 + 必要时提问并写回答案 + 生成报告或取消
qcloop：负责全局队列、断点恢复、状态机、执行器失败重试、worker/verifier/repair、issue ledger、证据留存；repair 会拿到上一轮 stdout/stderr/exit_code 和质检反馈,用于修复后复测
```
...
一句话：**人只给意图和确认，agent 负责操作，qcloop 负责把 agent 检查变成可追踪的 QA loop。**
