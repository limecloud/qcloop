...
**执行流程**：
1. 全局 worker pool 领取测试项（serve 模式默认并发 2；CLI run 保持同步执行）
2. 对每个 item：
   - **Worker**: 替换 prompt 模板中的 `{{item}}`，调用 `executor_provider` 对应的本机 CLI
...
graph TB
    ...
    subgraph "执行层"
        Executor[Provider Adapter<br/>Codex / Claude / Gemini / Kiro]
    end
...
```mermaid
flowchart TD
    Start([开始执行 item]) --> Worker[Worker 执行<br/>provider CLI]
...
**执行器**：
- Provider Adapter（子进程调用）
- 支持 `codex` / `claude_code` / `gemini_cli` / `kiro_cli`
...
- ✅ 可以通过 CLI 或 Web 创建批次
- ✅ 批次包含多个测试项（逗号分隔）
- ✅ 系统通过全局 worker pool 执行所有测试项，默认并发 2 个 item
- ✅ 可以查看每个测试项的状态
- ✅ 可以查看整体统计（成功/失败/进行中）
...
│  ├───────────────────────────────────────────────────────┤  │
│  │ test-1   │ abc123 │ 🟢通过│ 10项  │ 最多3轮 │ 05-09 23:00 │ 05-09 23:05 │ 查看详情 │
...
│   │   └── runner.go   # 多轮质检逻辑
│   ├── executor/       # 执行器
│   │   ├── provider.go # provider 选择
│   │   ├── agent_cli.go # Claude/Gemini/Kiro CLI 调用
...
### Stage 3: 执行器（已完成）
- ✅ Provider Adapter
- ✅ Codex / Claude Code / Gemini CLI / Kiro CLI 子进程调用
- ✅ stdout/stderr 捕获
...
- [ ] 任务优先级
- [x] ~~多 CLI executor provider(Codex / Claude Code / Gemini CLI / Kiro CLI)~~(已完成)

### 10.4 明确不做

- **Celery / Redis / RabbitMQ 等异步任务框架**:当前是单机 Go 程序,SQLite + goroutine 已经覆盖队列语义。真要分布式,优先 Go 原生方案(如 Asynq),不引入 broker。
...
**Q: 为什么默认只并发 2 个 worker？**
A: qcloop 主要驱动本机 AI 子进程,默认 2 个 worker 能提升吞吐,又避免模型限流和本机资源争用;可通过 `--workers` 或 `QCLOOP_WORKER_COUNT` 调整。

**Q: 如何选择 executor？**
A: 批次字段 `executor_provider` 可选 `codex` / `claude_code` / `gemini_cli` / `kiro_cli`；未传时使用 `QCLOOP_EXECUTOR_PROVIDER`，再退回 `codex`。
